...
def scrape_and_export():
    ...
    # Initialize SI scraper
    si_scraper = SIScraper(db)
    
    try:
        print("Running SI scraper...")
        si_scraper.scrape()
    except Exception as e:
        print(f"Error running SI scraper: {str(e)}")
    
    # Read SI events from MongoDB
    all_events = []
    
    # Read academic events from SI only
    academic_events = db.academic_events.find({'resource_source': 'CMU SI Page'})
    for resource in academic_events:
        for event in resource.get('events', []):
            ...
            print(f"Added SI event: {event_data}")
    
    # Convert to DataFrame and save to Excel
    if all_events:
        df = pd.DataFrame(all_events)
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        output_file = os.path.join(os.path.dirname(os.path.dirname(__file__)), f'si_events_{timestamp}.xlsx')
        df.to_excel(output_file, index=False)
        print(f"\nEvents exported to {output_file}")
        print(f"Total events exported: {len(all_events)}")
    else:
        print("No SI events were found to export")
